...
const	URL = { // Objeto con las diferentes partes de la URL de la API
		...
		procesarDatosPersonaje = (datos) => { // Función que procesará la información obtenida de la llamada a la API
			...
			document.body.querySelector('section').appendChild(elementoPárrafo); // Movemos el párrafo y su texto al section dentro de body
		};
...
function infoPersonaje (ID) { // Función que dependiendo de la ID dada, o 1 en su defecto, hará la llamada a la API para obtener los datos relacionados
	const	idPersonaje = Number(ID) || 1, // ID a buscar con la API
			...
			opcionesLlamada = { crossDomain: true /** Evitamos problemas al hacer llamadas a una API dentro de un dominio diferente al nuestro */ }; // Opciones para el método de jQuery que hará la llamada asíncrona a la API

	return $.get(urlLlamada, opcionesLlamada, procesarDatosPersonaje); // Método que hará la llamada asíncrona a la API y pasará los datos obtenido a la función 'procesarDatosPersonaje'
};
